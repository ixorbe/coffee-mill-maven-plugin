...
public class JavaScriptCompilerMojo extends AbstractCoffeeMillWatcherMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        if (getJavaScriptDir().isDirectory()) {
        	try {
                ...
                compile(getJavaScriptDir(), getWorkDirectory());
    ...
    private void compile(File input, File out) throws WatchingException {
        getLog().info("Compressing JavaScript files from " + input.getName() + " using Google Closure");
        ...
        Collection<File> files = FileUtils.listFiles(input, new String[]{"js"}, true);
        ...
        for (File file : files) {
            if (file.isFile() ) {
                store.add(new File(out.getAbsolutePath() + "/" + file.getName()));
